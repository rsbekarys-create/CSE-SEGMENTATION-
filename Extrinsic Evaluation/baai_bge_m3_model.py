# -*- coding: utf-8 -*-
"""BAAI_bge_m3_model.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1sP89_mTJpSq83jkAYetrSCIbDdRANDZX
"""

!pip install bert-score

"""BAAI bge m3 model with baseline dataset"""
...
#
# ✅ CHANGE:
#   - MODEL switched to BAAI/bge-m3
#   - Optional bge-m3 prefixes: "query: " and "passage: " (can disable)
# ============================================================
...
# bge-m3: prefixes often help retrieval quality
USE_PREFIX  = True
Q_PREFIX    = "query: "
P_PREFIX    = "passage: "
...
def apply_prefix(text: str, is_query: bool) -> str:
    if not USE_PREFIX:
        return text
    return (Q_PREFIX if is_query else P_PREFIX) + text
...
def build_index(model: SentenceTransformer, train_rows: List[Dict[str,str]]) -> QAIndex:
    ...
    # prefix (optional)
    q_view = [apply_prefix(q, is_query=True) for q in q_clean]
    a_view = [apply_prefix(a, is_query=False) for a in a_clean]
    ...
    return QAIndex(q_clean, q_emb, a_clean, a_emb)
...
def eval_run(model: SentenceTransformer, train_rows, test_rows) -> Tuple[Dict[str,Any], List[Dict[str,Any]]]:
    ...
    test_q_clean = [clean_view(x["question"]) for x in test_rows]
    test_q_view  = [apply_prefix(q, is_query=True) for q in test_q_clean]
    test_q_emb   = model.encode(test_q_view, convert_to_numpy=True, normalize_embeddings=True, show_progress_bar=False)

    gold_clean = [clean_view(x["answer"]) for x in test_rows]
    gold_view  = [apply_prefix(a, is_query=False) for a in gold_clean]
    gold_a_emb = model.encode(gold_view, convert_to_numpy=True, normalize_embeddings=True, show_progress_bar=False)
...
def interactive(model: SentenceTransformer, train_rows: List[Dict[str,str]]):
    ...
    while True:
        ...
        if q.lower() in {"exit","quit","q"}:
            break
        q_clean = clean_view(q)
        q_view  = apply_prefix(q_clean, is_query=True)
        qv = model.encode([q_view], convert_to_numpy=True, normalize_embeddings=True, show_progress_bar=False)[0]
...
def main():
    ...
    print(f"Model: {MODEL_NAME} | USE_PREFIX={USE_PREFIX} | DEVICE={DEVICE}")
