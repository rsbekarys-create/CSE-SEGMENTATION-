# -*- coding: utf-8 -*-
"""Fine tuning_MiniLM_Segmented_dataset
...
"""Fine tuning MiniLM Morph Segmented dataset"""

# @title
# ============================================================
# SEG_AS_BASE — MORPH_ONLY vs DUAL_VIEW_FUSED (clean+morph)  ✅
# + ✅ FINETUNING (5 epochs) added WITHOUT changing structure
#
...
#   - Trains bi-encoder on (morph_question -> morph_answer) pairs
#   - Loss: MultipleNegativesRankingLoss (in-batch negatives)
#   - Saves model locally, then runs same evaluation
#
...
# + CSV export per mode
#
# ✅ ONLY CHANGE REQUESTED (kept):
#   Interactive output answer is CLEANED at the end (removes @@ only for printing).
#   Model/index/retrieval/eval logic unchanged (fine-tune only adds training step).
# ============================================================
...
# ---------------------------
# ✅ FINETUNE CONFIG (5 epochs)
# ---------------------------
DO_FINETUNE        = True
FINETUNE_EPOCHS    = 5
FINETUNE_BATCH     = 32      # if OOM: try 16
FINETUNE_LR        = 2e-5
FINETUNE_WARMUP_FR = 0.10
FINETUNE_MAX_PAIRS = None    # None -> use all train pairs
SAVE_DIR           = "finetuned_miniLM_seg_morph_5ep"  # output dir
...
# ---------------------------
# ✅ FINETUNING (5 epochs) — added as a function (structure-safe)
# ---------------------------
def finetune_model(base_model_name: str, train_rows: List[Dict[str,str]], save_dir: str) -> SentenceTransformer:
    """
    Fine-tunes SentenceTransformer on (morph_question -> morph_answer) pairs.
    Uses MultipleNegativesRankingLoss (in-batch negatives).
    Saves to save_dir and returns the fine-tuned model.
    """
    # Lazy imports to keep top structure unchanged
    import torch
    from torch.utils.data import DataLoader
    from sentence_transformers import InputExample, losses

    # If model already exists, just load it (deterministic reuse)
    if Path(save_dir).exists() and any(Path(save_dir).glob("*")):
        print(f"\n✅ Found existing fine-tuned model: {save_dir} -> loading")
    ...
    samples = []
    for r in train_rows:
        q = morph_marker_view(r["question"])
        a = morph_marker_view(r["answer"])
        if q and a:
    ...
    train_loader = DataLoader(samples, shuffle=True, batch_size=FINETUNE_BATCH, drop_last=True)
    train_loss = losses.MultipleNegativesRankingLoss(model)

    warmup_steps = int(len(train_loader) * FINETUNE_EPOCHS * FINETUNE_WARMUP_FR)
    print(f"Train pairs: {len(samples)} | Steps/epoch: {len(train_loader)} | Warmup steps: {warmup_steps}")
    ...
    model.fit(
        ...
        use_amp=torch.cuda.is_available(),  # faster on GPU
    ...
    # Reload from disk for clean state
    return SentenceTransformer(save_dir, device=DEVICE)
...
def main():
    ...
    # ✅ Fine-tune or load fine-tuned model
    if DO_FINETUNE:
        model = finetune_model(MODEL_NAME, train_rows, SAVE_DIR)
        print(f"\nModel (fine-tuned): {SAVE_DIR}")
    else:
